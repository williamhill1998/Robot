...
'''---------This Class creates a thread that controls the TX that can be paused 
or resumed when desired---------'''
class write_thread(threading.Thread):
    ...
    def pause(self):
        self.paused = True
        self.pause_cond.acquire() #Locks the thread

    def resume(self):
        self.paused = False
        self.pause_cond.notify() #Releases the wait function
        self.pause_cond.release() #Unlocks the thread
